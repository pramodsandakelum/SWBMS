...
const createCustomIcon = (fullness) => {
    const getColor = (fullness) => {
        ...
        if (fullness >= 50) return '#F59E0B'; // Amber
        return '#3ECF8E'; // Emerald (matching theme primary)
    };
...
const BinMap = ({ bins }) => {
    ...
    const getMapCenter = () => {
        if (validBins.length === 0) {
            return [7.086789, 80.038834]; // fallback center
        }
    ...
    return (
        <Card className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/30 rounded-lg shadow-2xl">
            <CardContent className="p-0">
                ...
                {/* Statistics Bar - matching app design */}
                    ...
                    <div className="w-full h-[500px] rounded-lg overflow-hidden border border-slate-700/30 shadow-inner">
                        <MapContainer
                            ...
                            whenCreated={(mapInstance) => {
                                ...
                                // Set tighter bounds focused on the bin cluster
                                const focusedBounds = [
                                    [7.068, 80.020],  // Southwest corner (closer to bins)
                                    [7.098, 80.046]   // Northeast corner (closer to bins)
                                ];
